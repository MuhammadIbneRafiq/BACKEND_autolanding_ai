...
export const SearchTwitter = async (user) => {
  try {
    const email = user.email;
    const client = await supabaseClient.from("buyer_seller_table");
    ...
    if (data.role === "buyer") {
      return tweetResult; // right now its the list that we have from our leads at notion, later it will be webscraped(if we have validated that this is what freelancers/sellers will PAY for!)
